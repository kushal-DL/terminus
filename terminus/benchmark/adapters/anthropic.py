...
class AnthropicAdapter(LLMAdapter):
    ...
    def __init__(self, config: ModelConfig) -> None:
        ...
        if not self._endpoint.endswith("/messages"):
    ...
    async def get_action(
        ...
        messages = self._build_messages(history, turn_message)
        raw_text = await self._call_api(system_prompt, messages)
    ...
    def get_token_count(self, messages: list[Message]) -> int:
        ...
        return count_messages_tokens(messages, "anthropic", self.config.model)
    ...
    async def _call_api(self, system_prompt: str, messages: list[dict[str, str]]) -> str:
        ...
        if resp.status_code == 429:
            ...
            raise LLMError("api_error", "Anthropic API overloaded", status_code=529)
